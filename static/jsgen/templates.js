...
qs.question.statusbar = function(opt_data, opt_ignored) {
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(((opt_data.queueSize != null) ? '<p class="qs_statusbar_section"><b>Queue</b>:' + ((opt_data.queueSize == 0) ? 'No items pending (next ' + soy.$$escapeHtml(opt_data.timeUntilNextText) + ').' : (opt_data.queueSize == 1) ? '1 item pending.' : soy.$$escapeHtml(opt_data.queueSize) + ' items pending.') + '</p>' : '') + ((opt_data.answerPct != null && opt_data.answerTimeMs != null) ? '<p class="qs_statusbar_section"><b>Last answer</b>:' + soy.$$escapeHtml(opt_data.answerPct) + '% (after ' + soy.$$escapeHtml(opt_data.answerTimeMs) + 'ms)</p>' : ''));
};
if (goog.DEBUG) {
